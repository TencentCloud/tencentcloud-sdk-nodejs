...
提交训练写真模型任务按并发计费，无默认并发额度。
  ...
  async SubmitTrainPortraitModelJob(
    ...
    cb?: (error: string, rep: SubmitTrainPortraitModelJobResponse) => void
  ): Promise<SubmitTrainPortraitModelJobResponse> {
...
百变头像默认提供1个并发任务数，代表最多能同时处理1个已提交的任务，上一个任务处理完毕后才能开始处理下一个任务。
   ...
   * 查询图片特效任务
   */
  async DescribeTemplateToImageJob(
    req: DescribeTemplateToImageJobRequest,
    cb?: (error: string, rep: DescribeTemplateToImageJobResponse) => void
  ): Promise<DescribeTemplateToImageJobResponse> {
...
默认提供0个并发，代表最多能同时处理1个已提交的任务，上一个任务处理完毕后，才能开始处理下一个任务。
  ...
  async QueryTextToImageJob(
    req: QueryTextToImageJobRequest,
    cb?: (error: string, rep: QueryTextToImageJobResponse) => void
  ): Promise<QueryTextToImageJobResponse> {
...
默认提供1个并发，代表最多能同时处理1个已提交的任务，上一个任务处理完毕后，才能开始处理下一个任务。
     */
  async TextToImageRapid(
    req: TextToImageRapidRequest,
    cb?: (error: string, rep: TextToImageRapidResponse) => void
  ): Promise<TextToImageRapidResponse> {
...
默认提供1个并发，代表最多能同时处理1个已提交的任务，上一个任务处理完毕后，才能开始处理下一个任务。
     */
  async SubmitTextToImageJob(
    req: SubmitTextToImageJobRequest,
    cb?: (error: string, rep: SubmitTextToImageJobResponse) => void
  ): Promise<SubmitTextToImageJobResponse> {
   ...
   * 提交图片特效任务
   */
  async SubmitTemplateToImageJob(
    req: SubmitTemplateToImageJobRequest,
    cb?: (error: string, rep: SubmitTemplateToImageJobResponse) => void
  ): Promise<SubmitTemplateToImageJobResponse> {
...
每个写真模型自训练完成起1年内有效，有效期内可使用写真模型 ID 生成图片，期满后需要重新训练。
  ...
  async QueryDrawPortraitJob(
    ...
    cb?: (error: string, rep: QueryDrawPortraitJobResponse) => void
  ): Promise<QueryDrawPortraitJobResponse> {
