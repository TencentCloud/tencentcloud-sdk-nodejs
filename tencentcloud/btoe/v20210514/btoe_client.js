...
const VerifyEvidenceHashResponse = models.VerifyEvidenceHashResponse;
...
const VerifyEvidenceBlockChainTxHashRequest = models.VerifyEvidenceBlockChainTxHashRequest;
const VerifyEvidenceHashRequest = models.VerifyEvidenceHashRequest;
const CreateHashDepositNoCertResponse = models.CreateHashDepositNoCertResponse;
const VerifyEvidenceBlockChainTxHashResponse = models.VerifyEvidenceBlockChainTxHashResponse;
...
class BtoeClient extends AbstractClient {
    ...
    constructor(credential, region, profile) {
        super("btoe.tencentcloudapi.com", "2021-05-14", credential, region, profile);
    }
...
-注：Hash类存证、业务数据明文存证不产生存证文件。
     ...
     * @param {VerifyEvidenceBlockChainTxHashRequest} req
     * @param {function(string, VerifyEvidenceBlockChainTxHashResponse):void} cb
     * @public
     */
    VerifyEvidenceBlockChainTxHash(req, cb) {
        let resp = new VerifyEvidenceBlockChainTxHashResponse();
        this.request("VerifyEvidenceBlockChainTxHash", req, resp, cb);
    }

    /**
     * 用户存证内容hash向BTOE核验存证记录的真实性。
     * @param {VerifyEvidenceHashRequest} req
     * @param {function(string, VerifyEvidenceHashResponse):void} cb
     * @public
     */
    VerifyEvidenceHash(req, cb) {
        let resp = new VerifyEvidenceHashResponse();
        this.request("VerifyEvidenceHash", req, resp, cb);
    }
